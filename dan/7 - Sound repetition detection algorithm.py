...
def predict(path):
  ...
  theta_dtw = 0.5
  i = 0
  distances = [100] * len(times)

  while i < len(times) - W:
    seg1 = M[:, i:i + W//2]
    seg2 = M[:, i + W//2:i + W]

    if np.mean(rms_db[0, i:i + W//2]) > 55 and np.mean(rms_db[0, i + W//2:i + W]) > 55:
      i += 1
      continue

    D, _ = lr.sequence.dtw(X=seg1, Y=seg2)
    distances[i] = 100 * lr.util.normalize(D)[-1, 1]

    if distances[i] < theta_dtw:
      repetitions.append([i, i + W])
      i += W
...
with open("Results 7.csv", "w") as file:
  print("Clip,SoundRep", file=file)
