...
def predict(path):
  ...
  f0, voiced_flag, voiced_prob = lr.pyin(y=y, fmin=81, fmax=300, sr=sr, hop_length=hop_length)
  ...
  for j in range(len(times)):
    rms_limit = 55
    dissimilarity_limit = 7
    delta_f0 = f0[j] - f0[j - 1]

    if rms_db[0, j] > rms_limit and dissimilarity[j] < dissimilarity_limit and voiced_prob[j] > 0.5 and not math.isnan(delta_f0) and abs(delta_f0) < 15:
      continue
